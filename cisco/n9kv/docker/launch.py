...
class N9KV_vm(vrnetlab.VM):
    def __init__(self, hostname, username, password, conn_mode):
        ...
        super(N9KV_vm, self).__init__(
            username, password, disk_image=disk_image, ram=DEFAULT_RAM,
            smp=DEFAULT_SMP, cpu="host,level=9"
        )
    ...
    def bootstrap_spin(self):
        ...
        (ridx, match, res) = self.expect([b"\(yes\/skip\/no\)\[no\]:",b"\(yes\/no\)\[n\]:", b"\(yes\/no\)\[no\]:", b"login:"], 1)
        if match:  # got a match!
            if ridx in (0, 1, 2):
                self.logger.info("matched poap prompt")
                ...
                self.logger.info(f'trying to log in with "admin" / {self.password}')
