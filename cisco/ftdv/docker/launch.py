...
class FTDV_vm(vrnetlab.VM):
    ...
    def bootstrap_spin(self):
        ...
        if not self.login_ready:
            (_, l_match, l_res) = self.expect([b"INFO: Power-On Self-Test"], 1)
            if l_match:
                self.logger.info("LOGIN READY")
            ...
            # booting, so let's give it some more time
                ...
                self.spins = 0
        elif self.login_ready or not self.install_mode:
            (ridx, match, _) = self.expect([b"login:"], 3)
            if match:  # got a match!
                if ridx == 0:  # login
                    ...
                    self.logger.info("matched, login:")
